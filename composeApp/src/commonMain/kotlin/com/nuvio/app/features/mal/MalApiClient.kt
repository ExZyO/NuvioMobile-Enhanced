...
internal object MalApiClient {
    ...
    private data class OptimisticMalEntry(
        val progress: Int,
        val timestamp: Long
    )
    
    private val optimisticCache = mutableMapOf<Int, OptimisticMalEntry>()
    ...
    suspend fun saveProgress(
        ...
        if (comments != null) {
            val encodedComments = comments.encodeURLParameter()
        ...
        return try {
            ...
            if (success) {
                optimisticCache[animeId] = OptimisticMalEntry(
    ...
    suspend fun fetchListEntry(
        ...
        val cacheBuster = getTimeMillis()
        val url = "$API_URL/anime/$animeId?fields=my_list_status%7Bstatus,score,num_episodes_watched,is_rewatching,start_date,finish_date,priority,num_times_rewatched,rewatch_value,tags,comments,updated_at%7D,num_episodes,main_picture,title&_cb=$cacheBuster"
        val headers = mapOf(
            "Authorization" to "Bearer $accessToken",
            "Accept" to "application/json",
        ...
        return try {
            ...
            val optimistic = optimisticCache[animeId]
            val finalProgress = if (optimistic != null && now - optimistic.timestamp < 5 * 60 * 1000) {
                if ((entry.progress ?: 0) < optimistic.progress) {
                    optimistic.progress
                } else {
                    entry.progress
                }
            ...
            return entry.copy(progress = finalProgress)
        } catch (_: Exception) {
            null
        }
    }
